...
type TxEncoder func(tx interface{}) ([]byte, error)

func DefaultJSONTxEncoder(cdc codec.ProtoCodecMarshaler) TxEncoder {
	return func(txx interface{}) ([]byte, error) {
		protoTx, ok := txx.(*tx.Tx)
		if ok {
...
func DefaultTxDecoder(cdc codec.ProtoCodecMarshaler) TxDecoder {
	return func(txBytes []byte) (interface{}, error) {
		...
		_, err = unknownproto.RejectUnknownFields(raw.BodyBytes, &body, true, cdc.InterfaceRegistry())
		if err != nil {
			return nil, sdkerrors.Wrap(sdkerrors.ErrTxDecode, err.Error())
		}
...
func DefaultJSONTxDecoder(cdc codec.ProtoCodecMarshaler) TxDecoder {
	return func(txBytes []byte) (interface{}, error) {
		var theTx tx.Tx
